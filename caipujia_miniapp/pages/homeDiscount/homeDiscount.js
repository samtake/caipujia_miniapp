...
import {
  HomeDiscount
} from 'homeDiscount-model.js';
var homediscount = new HomeDiscount();
...
Page({
  ...
  discountClasify: function () {
    var that = this
    homediscount.discountClasify(null, (res) => {
      that.setData({
        cates: res.cates
      })
      that.discountList(0);
  ...
  discountList: function (tid) {
    ...
    homeDiscount.discountList(req, (res) => {
      that.setData({
        list: res.list
      })
    })
  ...
  chooseClick: function (event) {
    var activityid = homediscount.getDataSet(event, 'activityid');
    var shopid = homediscount.getDataSet(event, 'shopid');
    wx.navigateTo({
